...
const rows = await page.evaluate(() => {
  const all = Array.from(document.querySelectorAll('.behavior-value-table-pane .value-row'));
  return all.slice(0, 6).map(r => Array.from(r.querySelectorAll('.value-col')).map(c => c.textContent.trim()));
});
