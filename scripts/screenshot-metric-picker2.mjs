...
const data = await page.evaluate(() => {
  ...
  return {
    ...
    bodyH: dialogBody ? Math.round(dialogBody.getBoundingClientRect().height) : 0,
    ...
    sideListMaxH: sideList ? getComputedStyle(sideList).maxHeight : 'none',
    sideListOverflow: sideList ? getComputedStyle(sideList).overflowY : 'none',
  };
});
console.log(JSON.stringify(data, null, 2));
const dialog = await page.$('.el-dialog');
if (dialog) await dialog.screenshot({ path: '/workspace/projects/public/sdk-screenshots/metric-picker-current2.png' });
await browser.close();
