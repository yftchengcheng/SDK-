...
const v = await page.evaluate(() => {
  ...
  const blockCenters = blocks.map(b => {
    ...
    return { y: r.top + r.height / 2, text: b.textContent.trim(), top: r.top, bottom: r.bottom };
  });
  return polys.map((p, i) => {
    ...
    const tipY = pts[1][1];
    // 找包含 tipY 的 block
    const containing = blockCenters.filter(b => tipY >= b.top - 20 && tipY <= b.bottom + 20);
    return { idx: i, tipX, tipY, target: containing.map(b => b.text).join('/'), color: p.getAttribute('fill') };
  });
});
