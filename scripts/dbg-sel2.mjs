...
const reg = await api('POST', '/api/v1/auth/register', {
  email: 'dss-' + Date.now() + '@x.com', password: 'Test123456',
...
// Look for the actual displayed label
const result = await page.evaluate(() => {
  const dlg = document.querySelector('.el-dialog')
  const sel = dlg?.querySelector('.el-form-item:nth-child(2) .el-select')
  // Find any text content in the select
  return {
    selText: sel?.textContent?.replace(/\s+/g, ' ').slice(0, 200),
    selHTML: sel?.innerHTML?.slice(0, 1200),
    hasPlaceHolder: !!sel?.querySelector('.el-input__placeholder'),
    placeHolderText: sel?.querySelector('.el-input__placeholder')?.textContent,
    selInputChildren: Array.from(sel?.querySelectorAll('*') || []).slice(0, 10).map(el => el.tagName + '.' + (el.className || '').slice(0, 30)).join('\n'),
  }
})
