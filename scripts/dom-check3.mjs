...
const r = await page.evaluate(() => {
  const th = document.querySelector('.el-table__header-wrapper th .cell');
  const td = document.querySelector('.el-table__body-wrapper td .cell');
  // 检查元素所有匹配的 css rule
  return {
    thInline: th?.getAttribute('style'),
    tdInline: td?.getAttribute('style'),
    thParentClass: th?.parentElement?.className,
    thParentParentClass: th?.parentElement?.parentElement?.parentElement?.parentElement?.parentElement?.className,
    thCellParentWidth: th?.parentElement?.getBoundingClientRect()?.width,
    tdCellParentWidth: td?.parentElement?.getBoundingClientRect()?.width,
  };
});
