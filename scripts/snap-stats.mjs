...
// 找 dataPreview card
const cards = await page.$$('.app-detail-card');
console.log('cards count:', cards.length);
for (let i=0; i<cards.length; i++) {
  const box = await cards[i].boundingBox();
  console.log(`card[${i}]:`, box);
}
// 找 dataPreview 卡片
const dataCard = await page.$('.app-detail-card:nth-child(2)');
if (dataCard) {
  const box = await dataCard.boundingBox();
  console.log('data card box:', box);
  await page.screenshot({ path: '/tmp/data_card.png', clip: { ...box, x: box.x - 2, y: box.y - 2, width: box.width + 4, height: box.height + 4 } });
}
await browser.close();
