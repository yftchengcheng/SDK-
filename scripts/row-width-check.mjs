...
const info = await page.evaluate(() => {
  ...
  const row = document.querySelector('.frequency-row:not(.frequency-row--header)');
  return {
    tblW: Math.round(tbl.getBoundingClientRect().width),
    tblCS_w: getComputedStyle(tbl).width,
    rowW: Math.round(row.getBoundingClientRect().width),
    rowCS_w: getComputedStyle(row).width,
    rowCS_display: getComputedStyle(row).display,
    rowCS_overflow: getComputedStyle(row).overflow,
  };
});
