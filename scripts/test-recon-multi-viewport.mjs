...
for (const [w, h] of [[1280, 800], [1440, 900], [1600, 900], [1920, 1080]]) {
  ...
  const result = await page.evaluate(() => {
    ...
    const opHeader = head[head.length - 1];
    const statusHeader = head[head.length - 2];
    const opOverlap = opHeader.left < statusHeader.left + statusHeader.w; // 是否和状态列重叠
    return {
      viewport: window.innerWidth + 'x' + window.innerHeight,
      headerCount: head.length, bodyRowCount: allRows.length, diffCount: diffs.length,
      diffs: diffs.slice(0, 5),
      wrap: { w: Math.round(wrap.getBoundingClientRect().width), scrollW: wrap.scrollWidth },
      opHeader, statusHeader, opOverlap
    };
  });
  console.log('===', w, 'x', h, '===');
