...
await page.screenshot({ path: '/tmp/list.png', clip: { x: box.x-2, y: box.y-2, width: box.width+4, height: box.height+4 } });
// 截 detail header
const header = await page.$('.app-detail-header');
const hbox = await header.boundingBox();
await page.screenshot({ path: '/tmp/header.png', clip: { x: hbox.x-2, y: hbox.y-2, width: hbox.width+4, height: hbox.height+4 } });
await browser.close();
