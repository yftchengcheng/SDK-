...
const v = await page.evaluate(() => {
  ...
  return {
    grid: { left: gridRect.left, right: gridRect.right, w: gridRect.width },
    blocks: blocks.map((b, i) => {
      const r = b.getBoundingClientRect();
      return {
        idx: i, text: b.textContent.trim(),
        left: Math.round(r.left - gridRect.left),
        right: Math.round(r.right - gridRect.left),
        top: Math.round(r.top - gridRect.top),
        bottom: Math.round(r.bottom - gridRect.top),
        centerX: Math.round(r.left - gridRect.left + r.width / 2),
        centerY: Math.round(r.top - gridRect.top + r.height / 2),
      };
    }),
    leftMetrics: leftMetrics.map((m) => {
      const r = m.getBoundingClientRect();
      return {
        name: m.querySelector('.funnel-metric-name').textContent.trim(),
        right: Math.round(r.right - gridRect.left),
        top: Math.round(r.top - gridRect.top),
        centerY: Math.round(r.top - gridRect.top + r.height / 2),
      };
