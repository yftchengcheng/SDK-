...
await wait(2000);
const list = await page.evaluate(() => Array.from(document.querySelectorAll('.docs-list-item-title')).map(i => i.textContent?.trim()));
console.log('API 参考 list:', JSON.stringify(list));
if (list[0]) {
  await page.evaluate(() => document.querySelector('.docs-list-item')?.click());
  await wait(1500);
  ...
  await page.screenshot({ path: '/workspace/projects/public/sdk-screenshots/api-ytad-after.png' });
}
await browser.close();
