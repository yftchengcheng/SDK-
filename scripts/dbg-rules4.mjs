...
const reg = await api('POST', '/api/v1/auth/register', {
  email: 'dr4-' + Date.now() + '@x.com', password: 'Test123456',
...
// Manually call the form's validate
const result = await page.evaluate(async () => {
  const dlg = document.querySelector('.el-dialog')
  const formEl = dlg?.querySelector('form')
  // Find the form ref by looking for ElForm component
  let comp = formEl?.__vueParentComponent
  while (comp && comp.type?.__name !== 'ElForm') comp = comp.parent
  if (!comp) return { error: 'no ElForm' }
  // Manually call validate with callback
  return new Promise(resolve => {
    comp.proxy.validate((valid, fields) => {
      resolve({ valid, fieldsCount: fields ? Object.keys(fields).length : 0, fields: fields ? Object.fromEntries(Object.entries(fields).map(([k, v]) => [k, v[0]?.message])) : null })
    })
  })
})
