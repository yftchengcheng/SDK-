...
const box = await page.$eval('.app-master-header', el => {
  const r = el.getBoundingClientRect();
  return { x: r.x, y: r.y, width: r.width, height: r.height };
});
console.log('header box:', box);
await page.screenshot({ path: '/tmp/header.png', clip: { ...box, x: box.x - 2, y: box.y - 2, width: box.width + 4, height: box.height + 4 } });
await browser.close();
