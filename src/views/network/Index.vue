<template>
  <div class="page-container">
    ...
    <el-dialog v-model="adapterDialog.show" :title="`Adapter 版本管理 - ${adapterDialog.networkName}`" width="900px" destroy-on-close>
      ...
      <el-table :data="adapterDialog.versions" v-loading="adapterDialog.loading" stripe size="small">
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" size="small" @click="downloadAdapter(row)">下载</el-button>
            <template v-if="row.status === 0">
              <el-button link type="success" size="small" @click="reviewAdapter(row, 1)">通过</el-button>
              <el-button link type="danger" size="small" @click="reviewAdapter(row, 2)">拒绝</el-button>
            </template>
            <el-button link type="danger" size="small" @click="deleteAdapter(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-dialog>
...
const reviewAdapter = async (row: any, status: number) => {
