...
### 10. 禁止粉饰执行结果（MUST NEVER VIOLATE）

**最严重违反条款。任何情况下禁止把 ❌ 解读为 ✅。**

- ❌ 禁止对用户说「全 OK」「代码修好了」当**任一项 FAILED**
- ❌ 禁止把 `4/6 PASSED` 解读为「基本 OK」
- ❌ 禁止忽略 `❌ 禁止标'完成'` 脚本输出仍然说「完成」
- ❌ 禁止用「理论上」「应该」推测代替真实验证

**正确做法**：
- 脚本 exit 1 → 告知用户**具体哪几项 ❌**
- 原样复制脚本输出（不改写、不总结）
- 把 ❌ 项**列为下一步必做**，不视为已完成
- 任何「应该 work」的推测 = **不算 PASSED**

**反例（这次犯的错）**：
- 自检 4/6 PASSED + 2/6 FAILED → 我说「全部 OK」❌
- 服务挂 + 接口 500 → 我说「代码全修好」❌
- `❌ 禁止标完成` 脚本输出 → 我忽略，仍然说「完成」❌
